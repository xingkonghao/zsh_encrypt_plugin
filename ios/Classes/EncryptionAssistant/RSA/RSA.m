...
+ (SecKeyRef)addPublicKey:(NSString *)key withTag:(NSString *)tag1{
    NSRange spos = [key rangeOfString:@"-----BEGIN PUBLIC KEY-----"];
    NSRange epos = [key rangeOfString:@"-----END PUBLIC KEY-----"];
    if(spos.location != NSNotFound && epos.location != NSNotFound){
        NSUInteger s = spos.location + spos.length;
...
+ (NSString *)encryptString:(NSString *)str publicKey:(NSString *)pubKey withTag:(NSString *)tag{
    NSData *data = [RSA encryptData:[str dataUsingEncoding:NSUTF8StringEncoding] publicKey:pubKey withTag:tag];
    NSString *ret = base64_encode_data(data);
    return ret;
}

+ (NSData *)encryptData:(NSData *)data publicKey:(NSString *)pubKey withTag:(NSString *)tag{
    if(!data || !pubKey){
        return nil;
    }
    SecKeyRef keyRef = [RSA addPublicKey:pubKey withTag:tag];
    if(!keyRef){
        return nil;
    }
    return [RSA encryptData:data withKeyRef:keyRef];
...
+ (NSString *)decryptString:(NSString *)str publicKey:(NSString *)pubKey withTag:(NSString *)tag{
    NSData *data = [[NSData alloc] initWithBase64EncodedString:str options:NSDataBase64DecodingIgnoreUnknownCharacters];
    data = [RSA decryptData:data publicKey:pubKey withTag:tag];
    NSString *ret = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    return ret;
...
+ (NSData *)decryptData:(NSData *)data publicKey:(NSString *)pubKey withTag:(NSString *)tag{
    if(!data || !pubKey){
        return nil;
    }
    SecKeyRef keyRef = [RSA addPublicKey:pubKey withTag:tag];
